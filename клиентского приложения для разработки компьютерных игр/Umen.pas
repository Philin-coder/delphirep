...
procedure TFmen.Button6Click(Sender: TObject);
var i:Integer;
var msel2:string;
begin
msel2:='select Rabotnik.N_Rab, Rabotnik.Fio,Dolshnost.Nam_dol, Rabotnik.Iphone,'+
'Rabotnik.St_rab,Rabotnik.Date_pr '+
'from Rabotnik '+
'join Dolshnost on Rabotnik.N_Dol=Dolshnost.N_Dol '+
'where Rabotnik.Date_yv is null and Rabotnik.St_rab not like'+QuotedStr('З%');

I:=DBGrid7.SelectedIndex;
try
dm.razdacha.Parameters.ParamByName('@id_r').Value:=dbgrid7.DataSource.DataSet.Fields[i].Value;
 MessageDlg('задача выдана ',mtInformation,[mbOK],0);
  dm.razdacha.ExecProc;
  dm.Queryfreeworkers.Close;
  dm.Queryfreeworkers.SQL.Clear;
    dm.Queryfreeworkers.SQL.text:=msel2;
  dm.Queryfreeworkers.Open;
...
procedure TFmen.DBLookupComboBox9Click(Sender: TObject);
var msel:string;
var msel2:string;
var i:Integer;
begin
msel:='select Zadanie.N_Zad,Zadanie.Sr_vip ,Rabotnik.Fio,Zadanie.St_rab,'+' '
+'Zadanie.Data_nash, Zadanie.Date_fakt, Proect.Nam_proect,Work.Nam_work'+' '+
'from Zadanie'+ ' '+
'join Rabotnik on Rabotnik.N_Rab=Zadanie.N_Rab'+' '+
'join Proect on Proect.N_Proekta=Zadanie.N_Proekta'+' '+
'join Work on Work.N_Work=Zadanie.N_Work'+' '+
'where Zadanie.Date_fakt is  null  AND Zadanie.St_rab   not LIKE'+QuotedStr('В%')+' '+
'and rabotnik.N_Rab='+' '+dM.Queryrabotnik.FieldByName('N_rab').AsString;

msel2:='select  Rabotnik.Fio,Dolshnost.Nam_dol, Rabotnik.Iphone,'+
'Rabotnik.St_rab,Rabotnik.Date_pr '+
'from Rabotnik '+
'join Dolshnost on Rabotnik.N_Dol=Dolshnost.N_Dol '+
'where Rabotnik.Date_yv is null and Rabotnik.St_rab not like'+QuotedStr('З%');
...
dm.Queryrz.close;
dm.QueryRZ.SQL.Clear;
dm.QueryRZ.SQL.Text:=msel;
dm.Queryrz.open;
dm.Queryfreeworkers.Close;
dm.Queryfreeworkers.SQL.Clear;
ShowMessage(msel2);
