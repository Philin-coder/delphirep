...
type
  TFrm_kind = class(TForm)
    ...
    procedure Upd_kind_btnClick(Sender: TObject);
    procedure kind_del_btnClick(Sender: TObject);
  private
...
procedure TFrm_kind.kind_del_btnClick(Sender: TObject);
var
  AreFieldsEmpty: Boolean;
begin
  AreFieldsEmpty:=(
  (kind_del_inp_dbl.Text='')
  );
...
procedure TFrm_kind.Upd_kind_btnClick(Sender: TObject);
const
  AllowedChars: TSysCharSet = ['А'..'Я', 'а'..'я', '0'..'9', ' ', '-', '.'];
var
  AreFieldsEmpty: Boolean;
...
begin
  AreFieldsEmpty:=(
  (Trim(upd_kind_inp.Text)='')or
  (upd_kind_id_dbl.Text='')
  );
  AreFieldsValid:=(
  ValidateComponentText(  upd_kind_inp,AllowedChars)
  );
  if AreFieldsEmpty or not AreFieldsValid then
  begin
    MessageDlg('Ошибка: одно из полей пустое или текст не прошел проверку.',
      ...
      begin
        if not dm.Connection.Connected then
          raise EDatabaseError.Create('Соединение с базой не установлено',4001);
           Parameters.ParamByName('@doc_kind_id').Value :=
           dm.kindQuery.FieldByName('doc_kind_id').AsString;
           Parameters.ParamByName('@doc_kind_naim').Value:=upd_kind_inp.Text;
           ExecProc;
           dm.kindQuery.Close;
           dm.kindQuery.Open;
        MessageDlg('Изменения внесены', mtInformation, [mbOK], 0);
