...
begin
  try
    UpdateFormProperties('Frm_doc', 'Информация о документе',
      clBtnFace, 1024, 768);
    Frm_doc.ShowModal;
  except
    on E: Exception do
    begin
      if not (E is EMyException) then
        raise EDatabaseError.Create('Ошибка при работе с докумениа', 3001);
      HandleException(E);
      if Assigned(Frm_doc) then
        Frm_doc.Free;
      raise;
    end;
  end;
...
procedure TFrm_main.Exempl_itemClick(Sender: TObject);
begin
  try
    UpdateFormProperties('frm_ex', 'Информация об экземпляре',
      clBtnFace, 1024, 768);
    Frm_ex.ShowModal;
  except
    on E: Exception do
    begin
      if not (E is EMyException) then
        raise EDatabaseError.Create('Ошибка при работе с формой  экземпляра', 3001);
      HandleException(E);
      if Assigned(Frm_ex) then
        Frm_ex.Free;
      raise;
    end;
  end;
