...
type
  TFrm_spec = class(TForm)
    ...
    procedure spec_del_btnClick(Sender: TObject);
  private
...
procedure TFrm_spec.spec_del_btnClick(Sender: TObject);
var
  AreFieldsEmpty: Boolean;
begin
  AreFieldsEmpty:=(
  (sp_del_spec_DBL.Text='')
  );
...
begin
  try
    if not DM.Connection.Connected then
      raise EDatabaseError.Create('Соедмнение с базой не установлено ',4001);
