...
type
  TFrm_grade = class(TForm)
    ...
    grade_del_inp_Box: TGroupBox;
    grade_del_btn_Box: TGroupBox;
    grade_del_data_Box: TGroupBox;
    grade_del_btn: TButton;
    del_garde_DBL: TDBLookupComboBox;
    del_grade_lBL: TStaticText;
    ins_grade_Grid: TDBGrid;
    grade_upd_Grd: TDBGrid;
    grade_del_Grid: TDBGrid;
    ...
    procedure grade_del_btnClick(Sender: TObject);
  private
...
procedure TFrm_grade.grade_del_btnClick(Sender: TObject);
var
...
begin
  ...
  try
      with dm.del_grade do
      begin
        if not dm.Connection.Connected then
          raise EDatabaseError.Create('Соединение с базой не установлено',4001);
           Parameters.ParamByName('@id_grade').Value :=
           dm.gradeQuery.FieldByName('id_grade').AsString;
           ExecProc;
           dm.gradeQuery.Close;
           dm.gradeQuery.Open;
