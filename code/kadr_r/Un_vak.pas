...
procedure TFrm_vak.Upd_vak_st_BtnClick(Sender: TObject);
begin
with dm.doQuery do
           begin
           close;
           sql.Clear;
           if vak_stBox.ItemIndex=0 then
           begin
           SQL.Text:='Update vakansia set  vak_st='
            +QuotedStr(IntToStr(0))+','+'quantity=quantity+1'+
            'where vakansia.id_vakans='+
            dm.vakQuery.FieldByName('id_vakans').AsString;
            ExecSQL;
           end
           else if vak_stBox.ItemIndex=1 then
                begin
                close;
           sql.Clear;
           SQL.Text:='Update vakansia set  vak_st='
            +QuotedStr(IntToStr(1))+','+'quantity=quantity-1'+
            'where vakansia.id_vakans='
            +dm.vakQuery.FieldByName('id_vakans').AsString;
            ExecSQL;
                end;
