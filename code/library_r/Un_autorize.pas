...
begin
     if MessageDlg('Вы действительно хотите закрыть форму?', mtConfirmation, [mbYes, mbNo], 0) = mrNo then
  begin
    CanClose := False; // Отменяем закрытие формы
    Exit;
  end;
...
begin
login_inp.Text:='Логин';
paswd_inp.Text:='Пароль';
end;
