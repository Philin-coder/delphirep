...
type
  TFrm_oper = class(TForm)
    ...
    procedure o_upd_BtnClick(Sender: TObject);
    procedure o_del_btnClick(Sender: TObject);
  private
...
procedure TFrm_oper.o_del_btnClick(Sender: TObject);
var
  AreFieldsEmpty: Boolean;
begin
  AreFieldsEmpty:=(
...
procedure TFrm_oper.o_upd_BtnClick(Sender: TObject);
var
  AreFieldsEmpty: Boolean;
begin
  AreFieldsEmpty:=(
      ...
      begin
        if not Connection.Connected then
          raise Exception.Create('Соединение с базой не установлено');
           Parameters.ParamByName('@oper_id').Value :=
           dm.operQuery.FieldByName('oper_id').AsString;
           Parameters.ParamByName('@smdata').Value:=oper_sm_data_inp.Date;
           ExecProc;
           dm.operQuery.Close;
           dm.operQuery.Open;
        MessageDlg('Изменения внесены', mtInformation, [mbOK], 0);
