...
type
  TFrm_men = class(TForm)
    ...
    procedure men_upd_btnClick(Sender: TObject);
...
procedure TFrm_men.men_del_btnClick(Sender: TObject);
var
...
procedure TFrm_men.men_upd_btnClick(Sender: TObject);
const
  AllowedChars: TSysCharSet = ['А'..'Я', 'а'..'я', '0'..'9', ' ', '-', '.'];
var
  AreFieldsEmpty: Boolean;
...
begin
  AreFieldsEmpty:=(
  (men_upd_u_dbl.Text='')or
  (Trim(men_u_fio_inp.Text)='')
  );
  AreFieldsValid:=(
  ValidateComponentText(men_u_fio_inp,AllowedChars)
  );
  if AreFieldsEmpty or not AreFieldsValid then
  begin
    MessageDlg('Ошибка: одно из полей пустое или текст не прошел проверку.',
      ...
      begin
        if not dm.Connection.Connected then
          raise EDatabaseError.Create('Соединение с базой не установлено',4001);
           Parameters.ParamByName('@Id_manager').Value :=
           dm.menQuery.FieldByName('Id_manager').AsString;
           Parameters.ParamByName('@fio').Value:=men_u_fio_inp.Text;
           ExecProc;
           dm.menQuery.Close;
           dm.menQuery.Open;
        MessageDlg('Изменения внесены', mtInformation, [mbOK], 0);
