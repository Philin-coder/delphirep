...
procedure TFrm_good.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if (key=112) then
begin
...
begin
  ShowMessage('Ошибка: введено некорректное значение цены');
  Exit;
end;
...
with dm.ins_good do
      begin
        if not Connection.Connected then
          raise Exception.Create('Соединение с базой не установдено');
