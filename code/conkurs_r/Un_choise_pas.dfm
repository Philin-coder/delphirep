object Frm_cript_choise: TFrm_cript_choise
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = #1060#1086#1088#1084#1072' '#1074#1099#1073#1086#1088#1072' '#1096#1080#1092#1088#1086#1074#1072#1085#1080#1103' '#1076#1083#1103' '#1087#1072#1088#1086#1083#1103
  ClientHeight = 808
  ClientWidth = 1018
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object cript_inp_box: TGroupBox
    Left = 0
    Top = 0
    Width = 1018
    Height = 121
    Align = alTop
    Caption = #1042#1099#1073#1086#1088' '#1096#1080#1092#1088#1086#1074#1072#1085#1080#1103' '#1076#1083#1103' '#1087#1072#1088#1086#1083#1103
    TabOrder = 0
    object oldpasBox: TGroupBox
      Left = 2
      Top = 72
      Width = 1014
      Height = 47
      Align = alBottom
      Caption = 'C'#1090#1072#1088#1099#1081' '#1087#1072#1088#1086#1083#1100
      TabOrder = 0
      object aldpasslbl: TLabel
        Left = 272
        Top = 16
        Width = 3
        Height = 13
      end
    end
    object pass_choiche_lbl: TStaticText
      Left = 3
      Top = 16
      Width = 137
      Height = 17
      Caption = #1042#1099#1073#1088#1072#1090#1100' '#1074#1080#1076' '#1096#1080#1092#1088#1086#1074#1072#1085#1080#1103
      TabOrder = 1
    end
    object passKindCombo: TComboBox
      Left = 2
      Top = 45
      Width = 1013
      Height = 21
      Align = alCustom
      Anchors = [akLeft, akRight]
      ItemHeight = 13
      TabOrder = 2
      OnChange = passKindComboChange
      Items.Strings = (
        'Random_base'
        #1064#1080#1092#1088' '#1062#1077#1079#1072#1088#1103
        'Morze'
        'Bloufish'
        'Hash')
    end
  end
  object cryortPC: TPageControl
    Left = 0
    Top = 121
    Width = 1018
    Height = 687
    ActivePage = hashTab
    Align = alClient
    TabOrder = 1
    OnChange = cryortPCChange
    object RandombaseTab: TTabSheet
      Caption = #1064#1080#1092#1088#1086#1074#1072#1085#1080#1077' '#1085#1072' '#1073#1072#1079#1077' '#1043#1077#1085#1077#1088#1072#1090#1086#1088#1072' '#1089#1083#1091#1095#1072#1081#1085#1099#1093' '#1095#1080#1089#1077#1083
      object rb_input_box: TGroupBox
        Left = 0
        Top = 0
        Width = 1010
        Height = 177
        Align = alTop
        Caption = #1042#1074#1086#1076' '#1076#1072#1085#1085#1099#1093
        TabOrder = 0
        DesignSize = (
          1010
          177)
        object rnd_base_out_pass: TLabeledEdit
          Left = 10
          Top = 108
          Width = 997
          Height = 21
          Anchors = [akLeft, akRight]
          EditLabel.Width = 72
          EditLabel.Height = 13
          EditLabel.Caption = #1053#1086#1074#1099#1081' '#1087#1072#1088#1086#1083#1100
          TabOrder = 0
        end
        object rnd_base_sym_amoount_inp: TLabeledEdit
          Left = 26
          Top = 56
          Width = 981
          Height = 21
          Anchors = [akLeft, akRight]
          EditLabel.Width = 155
          EditLabel.Height = 13
          EditLabel.Caption = #1050#1086#1083#1080#1095#1077#1089#1090#1074#1086' '#1089#1080#1084#1086#1074#1086#1083#1086#1074' '#1087#1072#1088#1086#1083#1103
          TabOrder = 1
          Text = '0'
        end
        object rnd_base_grader: TUpDown
          Left = 10
          Top = 56
          Width = 17
          Height = 21
          AlignButton = udLeft
          Associate = rnd_base_sym_amoount_inp
          TabOrder = 2
        end
      end
      object rnd_base_btn_box: TGroupBox
        Left = 0
        Top = 612
        Width = 1010
        Height = 47
        Align = alBottom
        TabOrder = 1
        object rnd_base_init_Btn: TButton
          Left = 324
          Top = 19
          Width = 293
          Height = 25
          Caption = #1043#1077#1085#1077#1088#1080#1088#1086#1074#1072#1090#1100' '#1087#1072#1088#1086#1083#1100
          TabOrder = 0
          OnClick = rnd_base_init_BtnClick
        end
      end
      object imgBox: TGroupBox
        Left = 0
        Top = 177
        Width = 1010
        Height = 435
        Align = alClient
        TabOrder = 2
        object cryptImg: TImage
          Left = 2
          Top = 15
          Width = 1006
          Height = 418
          Align = alClient
          ExplicitLeft = 248
          ExplicitTop = 192
          ExplicitWidth = 105
          ExplicitHeight = 105
        end
      end
    end
    object CezarTab: TTabSheet
      Caption = #1064#1080#1092#1086#1088#1074#1072#1085#1080#1077' '#1087#1088#1080' '#1087#1086#1084#1086#1096#1080' '#1096#1080#1088#1072' '#1062#1077#1079#1072#1088#1103
      ImageIndex = 1
      object cryprmainBox: TGroupBox
        Left = 0
        Top = 0
        Width = 1010
        Height = 305
        Align = alTop
        Caption = #1042#1074#1086#1076' '#1076#1072#1085#1085#1099#1093
        TabOrder = 0
        object cryptcondBox: TGroupBox
          Left = 2
          Top = 15
          Width = 1006
          Height = 58
          Align = alTop
          Caption = #1042#1093#1086#1076#1085#1099#1077' '#1091#1089#1083#1086#1074#1080#1103
          TabOrder = 0
          object Radioruslng: TRadioButton
            Left = 96
            Top = 16
            Width = 113
            Height = 17
            Caption = #1056#1091#1089#1089#1082#1080#1081' '#1090#1077#1082#1089#1090
            TabOrder = 0
            OnClick = RadioruslngClick
          end
          object Radioenglng: TRadioButton
            Left = 231
            Top = 16
            Width = 130
            Height = 17
            Caption = #1040#1085#1075#1083#1080#1081#1089#1082#1080#1081' '#1090#1077#1082'c'#1090
            TabOrder = 1
            OnClick = RadioenglngClick
          end
          object Radiomixed: TRadioButton
            Left = 416
            Top = 16
            Width = 113
            Height = 17
            Caption = #1057#1084#1077#1096#1072#1085#1085#1099#1081
            TabOrder = 2
            OnClick = RadiomixedClick
          end
        end
        object paramBox: TGroupBox
          Left = 2
          Top = 73
          Width = 1006
          Height = 230
          Align = alClient
          Caption = #1055#1072#1088#1072#1084#1077#1090#1088#1099
          TabOrder = 1
          DesignSize = (
            1006
            230)
          object orignPasinp: TLabeledEdit
            Left = 3
            Top = 56
            Width = 985
            Height = 21
            Anchors = [akLeft, akRight]
            EditLabel.Width = 93
            EditLabel.Height = 13
            EditLabel.Caption = #1048#1089#1093#1086#1076#1085#1099#1081'  '#1087#1072#1088#1086#1083#1100
            TabOrder = 0
          end
          object shift_inp: TLabeledEdit
            Left = 0
            Top = 105
            Width = 971
            Height = 21
            Anchors = [akLeft, akRight]
            EditLabel.Width = 31
            EditLabel.Height = 13
            EditLabel.Caption = #1057#1076#1074#1080#1075
            TabOrder = 1
            Text = '0'
          end
          object shift_grader: TUpDown
            Left = 971
            Top = 105
            Width = 17
            Height = 21
            Anchors = [akRight]
            Associate = shift_inp
            TabOrder = 2
          end
          object newPasinp: TLabeledEdit
            Left = 6
            Top = 161
            Width = 977
            Height = 21
            Anchors = [akLeft, akRight]
            EditLabel.Width = 75
            EditLabel.Height = 13
            EditLabel.Caption = #1053#1086#1074#1099#1081'  '#1087#1072#1088#1086#1083#1100
            TabOrder = 3
          end
        end
      end
      object cezarbtnBox: TGroupBox
        Left = 0
        Top = 596
        Width = 1010
        Height = 63
        Align = alBottom
        TabOrder = 1
        object cezarinitBtn: TButton
          Left = 268
          Top = 19
          Width = 705
          Height = 25
          Caption = #1056#1072#1089#1096#1080#1092#1088#1086#1074#1072#1090#1100
          TabOrder = 0
          OnClick = cezarinitBtnClick
        end
      end
      object cezar_imgBox: TGroupBox
        Left = 0
        Top = 305
        Width = 1010
        Height = 291
        Align = alClient
        TabOrder = 2
        object cezar_cryprt_img: TImage
          Left = 2
          Top = 15
          Width = 1006
          Height = 274
          Align = alClient
          ExplicitHeight = 349
        end
      end
    end
    object morze_tab: TTabSheet
      Caption = #1064#1080#1092#1088#1086#1074#1072#1085#1080#1077' '#1087#1088#1080' '#1087#1086#1084#1086#1097#1080' '#1072#1079#1073#1091#1082#1080' '#1052#1086#1088#1079#1077
      ImageIndex = 2
      object Morze_inp_Box: TGroupBox
        Left = 0
        Top = 0
        Width = 1010
        Height = 134
        Align = alTop
        Caption = #1042#1074#1086#1076' '#1076#1072#1085#1085#1099#1093
        TabOrder = 0
        object Radiom_rus_text: TRadioButton
          Left = 56
          Top = 56
          Width = 113
          Height = 17
          Caption = #1056#1091#1089#1089#1082#1080#1081' '#1090#1077#1082#1089#1090
          TabOrder = 0
          OnClick = Radiom_rus_textClick
        end
        object Radio_m_eng_text: TRadioButton
          Left = 304
          Top = 56
          Width = 113
          Height = 17
          Caption = #1040#1085#1075#1083#1080#1081#1089#1082#1080#1081' '#1090#1077#1082#1089#1090
          TabOrder = 1
          OnClick = Radio_m_eng_textClick
        end
        object Radio_m_mixed_text: TRadioButton
          Left = 584
          Top = 56
          Width = 113
          Height = 17
          Caption = #1057#1084#1077#1096#1072#1085#1085#1099#1081
          TabOrder = 2
          OnClick = Radio_m_mixed_textClick
        end
        object beeperPanel: TPanel
          Left = 2
          Top = 91
          Width = 1006
          Height = 41
          Align = alBottom
          TabOrder = 3
          object bipBtn: TBitBtn
            Left = 312
            Top = 12
            Width = 75
            Height = 25
            Caption = #1041#1080#1087#1077#1088
            TabOrder = 0
            OnClick = bipBtnClick
          end
        end
      end
      object morzebtinBox: TGroupBox
        Left = 0
        Top = 592
        Width = 1010
        Height = 67
        Align = alBottom
        TabOrder = 1
        object morzeinitBtn: TButton
          Left = 464
          Top = 32
          Width = 91
          Height = 25
          Caption = #1056#1072#1089#1096#1080#1092#1086#1088#1074#1072#1090#1100
          TabOrder = 0
          OnClick = morzeinitBtnClick
        end
      end
      object morzeParamBox: TGroupBox
        Left = 0
        Top = 134
        Width = 1010
        Height = 138
        Align = alTop
        Caption = #1055#1072#1088#1072#1084#1077#1090#1088#1099
        TabOrder = 2
        DesignSize = (
          1010
          138)
        object morignpas: TLabeledEdit
          Left = 0
          Top = 35
          Width = 985
          Height = 21
          Anchors = [akLeft, akRight]
          EditLabel.Width = 93
          EditLabel.Height = 13
          EditLabel.Caption = #1048#1089#1093#1086#1076#1085#1099#1081'  '#1087#1072#1088#1086#1083#1100
          TabOrder = 0
        end
        object m_new_pas: TLabeledEdit
          Left = 3
          Top = 81
          Width = 985
          Height = 21
          Anchors = [akLeft, akRight]
          EditLabel.Width = 75
          EditLabel.Height = 13
          EditLabel.Caption = #1053#1086#1074#1099#1081'  '#1087#1072#1088#1086#1083#1100
          TabOrder = 1
        end
      end
      object m_imageBox: TGroupBox
        Left = 0
        Top = 272
        Width = 1010
        Height = 320
        Align = alClient
        TabOrder = 3
        object m_Image: TImage
          Left = 2
          Top = 15
          Width = 1006
          Height = 303
          Align = alClient
          ExplicitLeft = 264
          ExplicitTop = 112
          ExplicitWidth = 105
          ExplicitHeight = 105
        end
      end
    end
    object BloufishTab: TTabSheet
      Caption = #1064#1080#1092#1088#1086#1074#1072#1085#1080#1077' '#1087#1088#1080' '#1087#1086' '#1084#1086#1096#1080' bloufish'
      ImageIndex = 3
      object bloufish_inp_Box: TGroupBox
        Left = 0
        Top = 0
        Width = 1010
        Height = 177
        Align = alTop
        Caption = #1042#1074#1086#1076' '#1076#1072#1085#1085#1099#1093
        TabOrder = 0
        DesignSize = (
          1010
          177)
        object blpasskeyInp: TLabeledEdit
          Left = 23
          Top = 77
          Width = 968
          Height = 21
          Anchors = [akLeft, akRight]
          EditLabel.Width = 67
          EditLabel.Height = 13
          EditLabel.Caption = #1050#1083#1102#1095' '#1087#1072#1088#1086#1083#1103
          TabOrder = 0
          Text = '0'
        end
        object blpaskeygrader: TUpDown
          Left = 7
          Top = 77
          Width = 14
          Height = 21
          AlignButton = udLeft
          Associate = blpasskeyInp
          TabOrder = 1
        end
        object blnewpas: TLabeledEdit
          Left = 10
          Top = 132
          Width = 997
          Height = 21
          Anchors = [akLeft, akRight]
          EditLabel.Width = 72
          EditLabel.Height = 13
          EditLabel.Caption = #1053#1086#1074#1099#1081' '#1087#1072#1088#1086#1083#1100
          TabOrder = 2
          OnExit = blnewpasExit
        end
        object blorignpas: TLabeledEdit
          Left = 10
          Top = 28
          Width = 997
          Height = 21
          Anchors = [akLeft, akRight]
          EditLabel.Width = 90
          EditLabel.Height = 13
          EditLabel.Caption = #1048#1089#1093#1086#1076#1085#1099#1081' '#1087#1072#1087#1086#1083#1100
          TabOrder = 3
        end
      end
      object blbtnBox: TGroupBox
        Left = 0
        Top = 608
        Width = 1010
        Height = 51
        Align = alBottom
        TabOrder = 1
        object blinitBtn: TButton
          Left = 432
          Top = 23
          Width = 75
          Height = 25
          Caption = #1056#1072#1089#1096#1080#1092#1086#1088#1074#1072#1090#1100
          TabOrder = 0
          OnClick = blinitBtnClick
        end
      end
      object blimgBox: TGroupBox
        Left = 0
        Top = 177
        Width = 1010
        Height = 431
        Align = alClient
        TabOrder = 2
        object blImg: TImage
          Left = 2
          Top = 15
          Width = 1006
          Height = 414
          Align = alClient
          ExplicitLeft = 336
          ExplicitTop = 168
          ExplicitWidth = 105
          ExplicitHeight = 105
        end
      end
    end
    object hashTab: TTabSheet
      Caption = #1064#1080#1092#1088#1086#1074#1072#1085#1080#1077' '#1087#1088#1080' '#1087#1086#1084#1086#1097#1080' hash'
      ImageIndex = 4
      object hash_inp_Box: TGroupBox
        Left = 0
        Top = 0
        Width = 1010
        Height = 105
        Align = alTop
        Caption = #1042#1074#1086#1076' '#1083#1072#1085#1085#1099#1093
        TabOrder = 0
        object hashUtfradio: TRadioButton
          Left = 23
          Top = 48
          Width = 113
          Height = 17
          Caption = 'UTF8'
          TabOrder = 0
          OnClick = hashUtfradioClick
        end
        object hash_asciiradio: TRadioButton
          Left = 368
          Top = 48
          Width = 113
          Height = 17
          Caption = 'Ascii'
          TabOrder = 1
          OnClick = hash_asciiradioClick
        end
      end
      object hash_paramBox: TGroupBox
        Left = 0
        Top = 105
        Width = 1010
        Height = 105
        Align = alTop
        Caption = #1055#1072#1088#1072#1080#1077#1090#1088#1099
        TabOrder = 1
        DesignSize = (
          1010
          105)
        object h_orignPas_inp: TLabeledEdit
          Left = 0
          Top = 35
          Width = 985
          Height = 21
          Anchors = [akLeft, akRight]
          EditLabel.Width = 93
          EditLabel.Height = 13
          EditLabel.Caption = #1048#1089#1093#1086#1076#1085#1099#1081'  '#1087#1072#1088#1086#1083#1100
          TabOrder = 0
        end
        object has_new_pas_inp: TLabeledEdit
          Left = 3
          Top = 73
          Width = 985
          Height = 21
          Anchors = [akLeft, akRight]
          EditLabel.Width = 75
          EditLabel.Height = 13
          EditLabel.Caption = #1053#1086#1074#1099#1081'  '#1087#1072#1088#1086#1083#1100
          TabOrder = 1
        end
      end
      object has_btnBox: TGroupBox
        Left = 0
        Top = 600
        Width = 1010
        Height = 59
        Align = alBottom
        TabOrder = 2
        object has_init_btn: TButton
          Left = 408
          Top = 24
          Width = 75
          Height = 25
          Caption = 'C'#1075#1077#1085#1077#1088#1080#1088#1086#1074#1072#1090#1100
          TabOrder = 0
        end
      end
      object has_imgBox: TGroupBox
        Left = 0
        Top = 210
        Width = 1010
        Height = 390
        Align = alClient
        TabOrder = 3
        object hash_img: TImage
          Left = 2
          Top = 15
          Width = 1006
          Height = 373
          Align = alClient
          ExplicitLeft = 280
          ExplicitTop = 168
          ExplicitWidth = 105
          ExplicitHeight = 105
        end
      end
    end
  end
end
