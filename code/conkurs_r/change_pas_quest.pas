...
Frm_cript_choise.u_number:=dm.SelbyIDQuery.FieldByName('id_user').Value;
    ...
    try
      ...
      begin
        ResultString :=
            ...
            'Вы действительно хотите изменить пороль'+' '+
            VarToStr(dm.SelbyIDQuery.FieldByName('user_pas').Value)+'?';
            greatingPanel.ShowHint:=true;
            greatingPanel.Hint:=
            'Для этого, Вам необходимо будет решить маленьекую задачку,'+#10#13+
            'с которой человек легко справится, а бот нет)';
