...
type
  ...
  public
    ...
    constructor Create(const Msg: string; AErrorCode: Integer = 0); overload;
    constructor CreateFromADOError(E: EADOError); overload;
    constructor CreateFromOLEError(E: EOleException); overload;
  end;

  // Конкретные исключения
  EInvalidInput = class(EMyException);
  EDatabaseError = class(EMyException);
  EOleError = class(EMyException); // Новый класс для ошибок COM/OLE

// Процедура для стандартной обработки ошибок
...
procedure AdjustImageProperties(const AImage: TImage);
procedure UniformizeComponentAnchors(AComponent: TComponent);
procedure UpdateFormShowHint(const FormName: string);
procedure UpdateControlsShowHint(ParentControl: TWinControl);
procedure UpdateAnchors(AControl: TControl);
procedure DisableLoginPromptsOnDataModule(const DataModuleName: string);
function FindDMByName(const mName: string): TDataModule;
function IsWinRARInstalled: Boolean;
procedure ArchiveSelectedFolderWithWinRAR;
function SaveToSSA(const FileIn, FileOut: string;
Compression: TCompressionLevel): Boolean;
function LoadFromSSA(const FileIn: string): Boolean;
type
  TMaskStatus = (msCustom, msMobilePhone, msRegularPhone);
  procedure SetMaskEditMask(AForm: TForm; Status: TMaskStatus);
implementation
type
...
begin
  if not Assigned(AImage) then
    Exit;
  if (not AImage.Stretch) and (not AImage.AutoSize) then
  begin
    AImage.Stretch := True;
...
begin
  ...
  begin
    ...
    begin
      if not TControl(Form.Components[i]).ShowHint then
        TControl(Form.Components[i]).ShowHint := True;
      if Form.Components[i] is TWinControl then
        UpdateControlsShowHint(TWinControl(Form.Components[i]));
    end;
  end;
end;
procedure UpdateControlsShowHint(ParentControl: TWinControl);
...
begin
  ...
  begin
    if ParentControl.Controls[i] is TControl then
    begin
      if not TControl(ParentControl.Controls[i]).ShowHint then
        TControl(ParentControl.Controls[i]).ShowHint := True;
      if ParentControl.Controls[i] is TWinControl then
        UpdateControlsShowHint(TWinControl(ParentControl.Controls[i]));
    end;
  end;
end;
procedure UniformizeComponentAnchors(AComponent: TComponent);
...
begin
  ...
  begin
    Control := TControl(AComponent);
    if (Control is TLabeledEdit) or
       (Control is TEdit) or
       (Control is TDBLookupComboBox) or
       (Control is TDateTimePicker) or
       (Control is TDBComboBox) or
       (Control is TMaskEdit) or
       (Control is TComboBox) then
    begin
      if ([akLeft, akTop] * Control.Anchors) <> [akLeft, akTop] then
      begin
        raise Exception.Create('Начальное состояние Anchors для компонента ' + Control.Name +
          ' не соответствует ожидаемому.');
      end;
      UpdateAnchors(Control);
    end;
  end;
  if AComponent is TWinControl then
  begin
    for i := 0 to TWinControl(AComponent).ControlCount - 1 do
    begin
      UniformizeComponentAnchors(TWinControl(AComponent).Controls[i]);
    end;
  end;
end;
procedure UpdateAnchors(AControl: TControl);
...
constructor EMyException.Create(const Msg: string; AErrorCode: Integer);
begin
  inherited Create(Msg);
  FErrorCode := AErrorCode;
end;

constructor EMyException.CreateFromADOError(E: EADOError);
begin
  inherited Create(E.Message);
  FErrorCode := 2001; // Код для ошибок БД
end;

constructor EMyException.CreateFromOLEError(E: EOleException);
begin
  inherited Create(E.Message);
  FErrorCode := 3001; // Код для ошибок COM
end;
procedure HandleException(E: Exception);
var
  LogFilePath: string;
...
begin
  try
    ...
    LogFilePath := ExtractFilePath(ParamStr(0)) + 'error_log.txt';
    ...
    Writeln(LogFile, FormatDateTime('yyyy-mm-dd hh:nn:ss', Now) + ' - Ошибка: ' + E.ClassName);
    Writeln(LogFile, 'Сообщение: ' + E.Message);

    if E is EMyException then
      Writeln(LogFile, 'Код ошибки: ' + IntToStr(EMyException(E).ErrorCode))
    else if E is EOleException then
      Writeln(LogFile, 'Код ошибки COM: ' + IntToStr(EOleException(E).ErrorCode));

    Writeln(LogFile, '----------------------------------------');
    CloseFile(LogFile);

    // Вывод сообщения пользователю
    if E is EInvalidInput then
    ...
    begin
      ShowMessage('Не удалось записать ошибку в лог: ' + LogError.Message);
    end;
  end;
end;
procedure DisableLoginPromptsOnDataModule(const DataModuleName: string);
...
begin
  DataModule := FindDMByName(DataModuleName);
  if not Assigned(DataModule) then
  begin
    ShowMessage('Модуль данных "' + DataModuleName + '" не найден.');
    Exit;
  ...
  begin
    if DataModule.Components[I] is TADOConnection then
    begin
      ADOConn := TADOConnection(DataModule.Components[I]);
      try
        if ADOConn.LoginPrompt then
        begin
          ADOConn.LoginPrompt := False;
          //ShowMessageFmt('LoginPrompt отключён для "%s".', [ADOConn.Name]);
        end;
      except
        on E: Exception do
        begin
...
begin
  ...
  for I := 0 to Screen.DataModuleCount - 1 do
  begin
    if SameText(Screen.DataModules[I].Name, mName) then
    begin
      Result := Screen.DataModules[I];
      Exit;
    end;
...
var
  Reg: TRegistry;
  StandardPaths: array[0..1] of string;
  Path: string;
begin
  Result := False;
  ...
  try
    ...
    if Reg.OpenKeyReadOnly('SOFTWARE\WinRAR') then
    begin
      if Reg.ValueExists('exe64') or Reg.ValueExists('exe32') then
      begin
        Result := True;
        Exit;
      end;
...
{$HINTS OFF}
function SaveToSSA(const FileIn, FileOut: string; Compression: TCompressionLevel): Boolean;
var
...
begin
  ...
  try
    ...
    try
      InFile.Read(FileOutName, SizeOf(FileOutName));
      ...
      try
        ...
        OutFile.SaveToFile(ExtractFilePath(FileIn) + FileOutName);
      finally
        OutFile.Free;
      end;
    finally
...
procedure ApplyMaskToMaskEdit(MaskEdit: TMaskEdit; Status: TMaskStatus);
begin
  case Status of
    msCustom: // Произвольная маска
      MaskEdit.EditMask := '!99-99-99;1;_'; // Пример произвольной маски
    msMobilePhone: // Маска для мобильного телефона
      MaskEdit.EditMask := '+7 \(999\) 000-00-00;1;_';
    msRegularPhone: // Маска для обычного телефона
      MaskEdit.EditMask := '9-99-99;1;_';
  end;
end;
procedure FindAndSetMaskEditMasks(AControl: TWinControl; Status: TMaskStatus);
var
  i: Integer;
  Control: TControl;
begin
  ...
  begin
    ...
    if Control is TMaskEdit then
      ApplyMaskToMaskEdit(TMaskEdit(Control), Status);
    if Control is TWinControl then
      FindAndSetMaskEditMasks(TWinControl(Control), Status);
  end;
end;
procedure SetMaskEditMask(AForm: TForm; Status: TMaskStatus);
begin
  ...
  begin
    ShowMessage('Форма не назначена.');
    Exit;
  end;
  FindAndSetMaskEditMasks(AForm, Status);
end;
